...
export default function DashboardPage() {
  ...
  // State untuk Ringkasan Terpisah (Reguler vs Pre-Order)
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      setIsLoading(true);
      try {
        // 1. Tarik seluruh data transaksi
        const { data: trxData, error: trxError } = await supabase
          .from("transaksi")
          .select("id, kode_trx, total_omset, created_at")
        ...
        // 2. Tarik rincian detail_transaksi beserta relasi ke transaksi untuk cek kode_trx
        ...
        // Pisahkan ID transaksi reguler (TRX-) dan pre-order (PO-)
        const regulerTrxIds = new Set(
          trxData.filter((t) => !t.kode_trx.startsWith("PO")).map((t) => t.id),
        );
        const poTrxIds = new Set(
          trxData.filter((t) => t.kode_trx.startsWith("PO")).map((t) => t.id),
        );

        // Hitung Omset Reguler & PO
        ...
        detailData?.forEach((item) => {
          ...
          // Cek apakah detail ini milik TRX atau PO berdasarkan kode_trx-nya
        ...
        // 3. Setup Data Grafik Keseluruhan (Gabungan Omset & Laba Bersih Total)
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-zinc-900 transition-colors duration-300 pb-10">
      ...
      {/* Konten Utama */}
      <main className="p-4 md:p-6 max-w-7xl mx-auto space-y-8 pt-6">
        <div className="flex justify-between items-center">
          ...
          <Link
            href="/produk"
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-lg transition-colors shadow-md text-sm"
          >
            Kelola Produk &rarr;
          </Link>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-zinc-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700">
              ...
              <p className="text-xs text-gray-400 mt-2">Modal barang terjual</p>
            </div>
            <div className="bg-white dark:bg-zinc-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                Total Overhead (20%)
              ...
              <p className="text-xs text-gray-400 mt-2">Potongan laba kotor</p>
            </div>
            <div className="bg-white dark:bg-zinc-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-zinc-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700">
              ...
              <p className="text-xs text-gray-400 mt-2">Keuntungan final PO</p>
            </div>
          </div>
        </div>
